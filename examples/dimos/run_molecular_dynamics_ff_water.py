...
def run_npt(
    ...
    gromacs_system = dimos.ff.GromacsForceField(
        parameter_file='inputs/water.top',
        xyz_file='inputs/water.gro',
        cutoff=9.0,
        switch_distance=7.5,
    ...
    traj_writer = ASETrajectoryWriter('results/' + model + f"/{temperature}/water.traj", simulation, gromacs_system, write_velocities=True)
    md_logger = MDLogger('results/' + model + f"/{temperature}/water.csv", simulation, gromacs_system, write_interval=write_interval, write_density=True)
