...
def run_npt(
    ...
    gromacs_system = dimos.ff.GromacsForceField(
        parameter_file='inputs/crambin.top',
        xyz_file='inputs/crambin.gro',
        cutoff=9.0,
        switch_distance=7.5,
    ...
    traj_writer = ASETrajectoryWriter('results/' + model + "/crambin.traj", simulation, gromacs_system, write_velocities=True)
    md_logger = MDLogger('results/' + model + "/crambin.csv", simulation, gromacs_system, write_interval=write_interval, write_density=True)
