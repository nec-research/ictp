...
def run_npt(
    ...
    folder = Path('../../models')
    model = 'n_feats_64-batch_size_256-seed_0'
    ...
    system = ICTPSystem.from_folder_list(
        ...
        topology_file='inputs/crambin.top',
        structure_file='inputs/crambin.gro',
        new_pair_potential_config={'electrostatics': dict(method='spme', r_cutoff=9.0, alpha=0.3372060287522547, k_max=46, spline_order=5)},
        device='cuda:0',
        periodic=True,
        use_neighborlist=True,
        with_torch_compile=True,
    )
    positions = dimos.read_positions('inputs/crambin.gro')
    ...
    traj_writer = ASETrajectoryWriter('results/' + model + "/crambin.traj", simulation, system, write_velocities=True)
    md_logger = MDLogger('results/' + model + "/crambin.csv", simulation, system, write_interval=write_interval, write_density=True)
