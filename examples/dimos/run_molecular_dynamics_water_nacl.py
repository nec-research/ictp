...
def run_npt(
    ...
    system = ICTPSystem.from_folder_list(
        ...
        topology_file=f'inputs/water_{int(concentration * 100):03d}_molal.top',
        structure_file=f'inputs/water_{int(concentration * 100):03d}_molal.gro',
    ...
    traj_writer = ASETrajectoryWriter('results/' + model + f'/{int(concentration * 100):03d}/water.traj', simulation, system, write_velocities=True)
    md_logger = MDLogger('results/' + model + f'/{int(concentration * 100):03d}/water.csv', simulation, system, write_interval=write_interval, write_density=True)
