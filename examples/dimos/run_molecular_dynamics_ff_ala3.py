...
def run_npt(
    ...
    gromacs_system = dimos.ff.GromacsForceField(
        parameter_file='inputs/ala3.top',
        xyz_file='inputs/ala3.gro',
        cutoff=9.0,
        switch_distance=7.5,
    ...
    positions = dimos.read_positions('inputs/ala3.gro')
    ...
    traj_writer = ASETrajectoryWriter('results/' + model + '/ala3_eq.traj', simulation, gromacs_system, write_velocities=True)
    md_logger = MDLogger('results/' + model + '/ala3_eq.csv', simulation, gromacs_system, write_interval=write_interval, write_density=True)
