...
def run_pbmetad(
    ...
    traj_writer = ASETrajectoryWriter(f'results/{model}/pbmetad-default_grid-frequency_100/trp_{walker_id}.traj', simulation, gromacs_system, write_velocities=True)
    md_logger = MDLogger(f'results/{model}/pbmetad-default_grid-frequency_100/trp_{walker_id}.csv', simulation, gromacs_system, write_interval=write_interval, write_density=True)
...
if __name__ == '__main__':
    ...
    # 20 ns long simulations for each walker = 120 ns in total
    args = [
        (i % n_gpus, i, n_walkers, 1.0, 298.15, 1.0, 0.01, 100, True, 20000000, 500, torch.float64)
